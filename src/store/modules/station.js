...
const state = {
  ...
  alarmArrJava: [],
  alarmArrJavaShow: [],
  javaEventA: "/hbty/fySetupPLCA", //向JAVA发送模拟量事件
  javaEventD: "/hbty/fySetupPLCB", //向JAVA发送模拟量事件
};
...
const getters = {
  wtS7SplitData(state) {
    ...
    let processData = [];
    let SidValue = [];
    station = state.stationDataAndInfo.map((r) => r.Station);
    SidValue = state.stationDataAndInfo.map((r) => r.Sid);
    processData = state.stationDataAndInfo;
